    ...
    *,
    ...
    def model_init(self, *, vllm_config, prefix: str = ""):
        ...
        if int(getattr(config, "vision_n_layers", 0) or 0) > 0:
            # b12x fused MHC Gram kernel requires rms_eps == 1e-6 (hard-coded
            # in b12x.norm.mhc._impl._supports_fused_mhc_gram). The Vision-Exp
            # checkpoint ships rms_norm_eps=1e-20, which fails that guard with
            # "b12x_mhc_pre is served only by the fused Gram kernel". Normalize
            # to 1e-6: RMSNorm eps is a div-by-zero stabilizer, both are far
            # below the activation variance scale (negligible for bf16), and
            # it matches the b12x decode config the kernel was built for.
            # Guarded on vision_n_layers so text-only 0731 is unchanged.

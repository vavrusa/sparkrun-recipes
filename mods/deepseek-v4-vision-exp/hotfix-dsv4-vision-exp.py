...
def patch_encoding_text(source: str) -> tuple[str, str]:
    ...
    if "IMAGE_PLACEHOLDER" not in source:
        # Lineages whose deepseek_v4_encoding.py has no image-placeholder logic
        # have nothing to relax here. The Anemll image this hotfix was written
        # for rejects already-substituted placeholders at the encoder; a
        # lineage without them makes the encoding patch a no-op.
        return source, "skipped-no-placeholder"
...
def _write(path: Path, original: str, updated: str, status: str) -> None:
    ...
    elif status not in ("skipped", "skipped-no-placeholder"):
